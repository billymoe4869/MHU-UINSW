import { getAllEventAdmin } from "@/lib/service/event";
import DeleteEventBtn from "@/components/admin/button/DeleteEventBtrn";
import Link from "next/link";
import { MdEdit } from "react-icons/md";
import { PiPlusBold } from "react-icons/pi";

export default async function DaftarKegiatan() {
  const event = await getAllEventAdmin();
  return (
            ...
            {event.length === 0 ? (
              <tr>
                <td colSpan={4} className="text-center p-6 text-stone-400">
                  Belum ada event. Tambahkan event pertama!
                </td>
              </tr>
            ) : (
